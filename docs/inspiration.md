...
recurring tactical decisions that became the framework's five
ADRs. Each sibling line is independent in content; the framework
is downstream of all four.
...
## Operational origins of the five ADRs

Each of the framework's five ADRs traces to a recurring decision
...
2. The five ADRs in [`adr/`](adr/), in numerical order, for the tactical decisions.
