<!-- Generated: 2026-07-16 | Files scanned: project root + docs/ tree + scripts/ | Token estimate: ~1500 -->
...
└── docs/
    ...
    ├── manifesto.md                     eleven open questions the framework leaves unanswered
...
| Open questions | `docs/manifesto.md` | Eleven open questions. Adopters invited to extend. |
...
| ADR index | `docs/adr/README.md` | Twenty-three ADRs grouped: identifier-federation triplet (0001-0003) + maintenance-discipline pair (0004-0005) + LLM-first ingest decision (0006) + metric-rejection decision (0007) + diffusion-mechanism cluster (0008-0011; 0009 amending 0006 — revised 2026-08-19 to set the structured graph's criterion at consideration-set entry / entity resolution rather than citation lift, 0010 defining the vocabulary discipline 0008 names, 0011 building the measurement instrument 0008 demands, with Annex A (2026-08) adding calibration reading rules as ADR-0007-bound diagnostics) + channel-selection decision (0012, applying the enclosure axis to external listings: link-index entries only, four-condition host audit) + intrinsic-identifier decision (0013, adding a content-derived SWHID layer that complements the DOI layer and closes manifesto open question 4) + implementation-tracking decision (0014, the two-tier ledger + periodic gap-review for operating the framework over time) + license-selection decision (0015, selecting an artifact's license by its dominant audience: a public-domain dedication for machine-mined artifacts, a permissive software license for code, prohibiting non-commercial and no-derivatives terms) + genre-split-placement decision (0016, routing the canonical by genre: the essay genre's canonical is the version-controlled repository corpus on an intrinsic content-derived identifier, the paper genre's on the concept DOI, with syndicated copies bound by entity federation rather than canonical-URL tags; instantiates 0013's DOI-impractical-genre clause and complements 0015) + doctrine-hardening cluster (0017-0019, adding no new tactic surface but tightening existing commitments: 0017 collects manifesto open question 8 — a diagnostic signal and recovery for each of the three acknowledged failure modes, under a diagnostic-is-a-detector-not-a-metric caveat; 0018 procedurizes origin-claim falsifiability — test a priority claim against prior art before publishing and rescope to its narrowest defensible form; 0019 draws the structural-optimization-versus-content-authenticity boundary — optimize the transmission path, never the content, content deformation being a Layer 1 violation reinforced by 0007) + derivation-surface-onboarding decision (0020, onboarding idea-bearing repositories to two third-party derivation surfaces — a paraphrasing synthetic wiki used as a regurgitation-test diagnostic answered upstream by dense anchoring, and a verbatim documentation hub whose access-count badge is a measurement signal not a metric — blessing the derived views, declining an index-only code-library catalog as an artifact-type mismatch and self-hosted query infrastructure as friction; the derivation-axis counterpart to 0012) + self-sovereign-grounding decision (0021, extracted from a tactic's revocation rather than a deployment: after a community-governed authority record revoked the author's account as promotional at the aggregate-pattern level and mass-deleted every self-created entry, self-created authority-record federation is retired permanently; grounding surfaces are classified by revocation control — only self-sovereign layers may be load-bearing for the origin claim, third-party-governed grounding counts only when earned unprompted; revoked machine-readable edges are purged, dated history preserved, circumvention prohibited; the sharpest observed instance of manifesto open question 9) + audience-layer-split decision (0022, with 0014 the second operating-the-framework ADR: the strategy's accounting split into a doctrine layer — LLM-mediated primary audience, months-to-years, 0007's metric rejection intact — and an essay layer — the governed essay corpus, contemporary human readers as primary audience, days-to-weeks, reception signals legitimate to observe and steer by; amends 0007's scope clause by clause, boundary clauses keep essay signals out of doctrine decisions, opens manifesto open question 11) + empirical-layer-role decision (0023, closing manifesto open question 7 by structuring the hybrid: reference baseline as the layer's primary role and minimum bar, case-study narrative as a bounded secondary role marked as interpretation, validation evidence declined unless an artifact is designed as an experiment before the fact and explicitly designated as validation evidence — a role upgrade that is never silent, modeled on 0011's visible series break; the evidence-revises-principles flow unchanged, a preliminary observation remains sufficient to trigger normative revision). |
...
| Ecosystem index | `docs/skills/README.md` | Reproduces this line's ecosystem as a table: the four external **component skills** (`authorship-strategy-skill`, `release-doi`, `llms-txt-writer`, `jsonld-knowledge-graph`), the always-loaded **rule** (`authorship-strategy-rules`), and the **adjacent skills / complements** (`readme-writer`, `wikidata-federation`, `doctrine-corpus`, `existence-proof`, `einstein-arena`). Skill bodies are NOT copied; each repository is canonical for itself. The component-status criterion (the skill operationalizes content the doctrine explicitly names) is documented in the file. The full cross-line inventory lives in the hub `graph.jsonld` / concept index, pointed to but not duplicated. Restored from the hub's pre-simplification ecosystem table (hub commit `a288e01`). |
...
| AI relationship map | `graph.jsonld` | schema.org JSON-LD. Three axes + four layers + twenty-three ADRs + sibling lines + disjoint-vocabulary edge to AAP. |
...
```
thesis.md ──────► (cited by) ─► all 23 ADRs
...
- *I want to add a new sibling research line to the ecosystem.* → This is a hub-level operation; see the hub repository's (`shimo4228/shimo4228`) CLAUDE.md for the hub's editing triggers.
