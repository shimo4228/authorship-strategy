...
抽出された 9 つの戦術的判断は何か（4 つの DOI 登録 repo からなる研究エコシステムでの運用から）。
...
9 つの ADR はフレームワークから演繹されたものではなく、sibling エコシステムの運用から
...
戦略を評価したい場合: まず [`docs/thesis.md`](docs/thesis.md)、次に 9 つの ADR を番号順に。
非自明な入口が要るのは次の 2 経路:
